<!DOCTYPE html>
<html>

<head>
    <link rel="stylesheet" type="text/css" href="styles.css">

    <div class="header">
        <h1>Header</h1>
        <p>Resize the browser window to see the responsive effect.</p>
        <div class="topnav">
            <a href="#"><img class="logo_menu" src="imagenes/NBALogo.png">INICIO</a>
            <a href="#"><img class="logo_menu" src="imagenes/NBALogo.png">CLASIFICACIÓN</a>
            <a href="#"><img class="logo_menu" src="imagenes/NBALogo.png">ÚNETE</a>
            <a href="#"><img class="logo_menu" src="imagenes/NBALogo.png">NOTICIAS</a>
            <a href="#"><img class="logo_menu" src="imagenes/NBALogo.png">GALERÍA</a>
        </div>
    </div>
</head>

<body>
    <div class="div_principal">
        <div class="columna_principal">
            <h2>Column</h2>
            <p>Lorem ipsum dolor sit amet, consectetur adipiscing elit. Maecenas sit amet pretium urna. Vivamus
                venenatis velit nec neque ultricies, eget elementum magna tristique. Quisque vehicula, risus eget
                aliquam placerat, purus leo tincidunt eros, eget luctus quam orci in velit. Praesent scelerisque tortor
                sed accumsan convallis.</p>
        </div>

</body>

</html>